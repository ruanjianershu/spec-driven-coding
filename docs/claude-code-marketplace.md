...
> SDC is a lightweight spec-driven coding workflow for Claude Code. It helps developers discover uncertain requirements, capture changes, preserve SCN/REQ/AC traceability, confirm high-impact decisions, apply tasks, run delivery checks, and archive stable specs in local project files.

Use this longer description when a form allows more context:

> SDC packages a complete spec-driven development lifecycle into a small set of Claude Code commands and skills. It creates a local `.sdc/` workspace for specs, changes, standards, decisions, and reports; uses Discovery Gate to explore uncertain requirements before confirmed specs; preserves `SCN -> REQ -> AC -> task -> evidence` traceability; uses consent gates so high-impact AI suggestions do not become silent defaults; applies changes incrementally; runs combined validation, review, test, quality, bug, impact, and repo checks; and archives completed changes into stable project specs. SDC is local-first and prompt-only: it ships no MCP server, no telemetry, no background daemon, no default hooks, and no external service dependency.
