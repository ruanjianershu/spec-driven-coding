...
9. archive   将完成的 change 沉淀到 .sdc/specs/ 和 archive/，并判断是否更新长期知识库
```
...
用 SDC archive 归档这个变更，并判断是否需要沉淀 decisions、standards、AGENTS 或 reports
```
...
也就是说，用户不需要在 `archive`、`harness`、`standards`、`decisions` 之间自己判断。SDC 会给出“建议更新哪些长期资产、为什么、写到哪里”，但条件项必须等你明确 yes/no 后才写入。
...
| 新项目从 0 到 1 | 很适合 | 从第一天建立 `.sdc/`、开发规范、需求历史、SCN/REQ/AC 和测试先行习惯，避免项目很快变成“无文档遗留项目”。 |
...
- ✅ `REQ/AC/T###` 最终覆盖摘要
- ✅ 原始变更历史保留
- ✅ Knowledge Compact Gate：判断 decisions、standards、AGENTS、reports、project、project-cognition 是否需要更新
- ✅ 归档结论和下一步建议
